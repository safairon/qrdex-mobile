...
exports.update = function(timestamp, callback) {
    ...
    setTimeout(function() {
        ...
        if (res.timestamp - timestamp > 3e4) {
...
exports.commit = function(req, callback) {
    ...
    setTimeout(function() {
        ...
        req["new"] && _.each(req["new"], function(list, table) {
            var newList = {};
        ...
        req["update"] && (res.changeList["update"] = req["update"]);
        req["delete"] && (res.changeList["delete"] = req["delete"]);
        Ti.API.info("return:\r	" + JSON.stringify(res));
        callback(res);
    }, delay);
};