...
function applyChangeList(changeList) {
    changeList["delete"] && clearChangeList(changeList["delete"]);
    changeList["update"] && updateChangeList(changeList["update"]);
}
...
function clearChangeList(changeList) {
    _.each(changeList, function(idList, table) {
        ...
        if (collection) {
            ...
            db.execute("COMMIT");
            db.close();
        } else Ti.API.error("there is no collection for " + table);
    });
}
...
exports.startSyncronization = function() {
    syncronizationId = setInterval(syncronization, Ti.App.Properties.getInt("syncInterval", 1e4));
};

exports.stopSyncronization = function() {
    syncronizationId && clearInterval(syncronizationId);
};
